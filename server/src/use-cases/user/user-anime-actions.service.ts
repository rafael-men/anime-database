...
export class UserAnimeActionsService {
   ...
   async removeAnimeFromFavorites(userId: string, externalAnimeId: string): Promise<void> {
      await this.validateUser(userId);
      this.validateAnimeId(externalAnimeId);
   ...
   async addComment(
      ...
      if (!comment || !comment.trim()) {
         throw new ValidationException('Comment cannot be empty.', 'COMMENT_EMPTY');
      }
      ...
      return this.reviewRepository.save(
         this.reviewRepository.create({
            userId,
            externalAnimeId,
            rating: options?.rating !== undefined ? this.normalizeRating(options.rating) : 0,
            comment: text,
            watchedAt: options?.watchedAt ?? new Date(),
            isRewatch: options?.isRewatch ?? false,
            hasSpoilers: options?.hasSpoilers ?? false,
   ...
   private async validateUser(userId: string): Promise<User> {
      ...
      if (!user) {
         throw new ResourceNotFoundException(`User with id ${userId} not found.`, 'USER_NOT_FOUND');
      }
      return user;
   }
   ...
   private normalizeStatus(status?: WatchlistStatus): WatchlistStatus {
      ...
      if (!Object.values(WatchlistStatus).includes(value)) {
         throw new ValidationException('Invalid watchlist status.', 'WATCHLIST_STATUS_INVALID');
      }
