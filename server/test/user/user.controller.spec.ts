...
describe('UserController', () => {
   ...
   beforeEach(async () => {
      const moduleRef: TestingModule = await Test.createTestingModule({
         ...
         providers: [
            ...
            { provide: UserAnimeActionsService, useValue: userAnimeActionsServiceMock },
         ],
      })
   ...
   it('POST /users/:id/favorites should add anime to favorites', async () => {
      ...
      expect(userAnimeActionsServiceMock.addAnimeToFavorites).toHaveBeenCalledWith(
         'user-1', '1', 'WATCHING',
      );
      expect(response.body).toEqual(
   ...
   it('DELETE /users/:id/favorites/:animeId should remove a favorite', async () => {
      userAnimeActionsServiceMock.removeAnimeFromFavorites.mockResolvedValue(undefined);

      await request(app.getHttpServer())
         .delete('/users/user-1/favorites/1')
         .expect(204);

      expect(userAnimeActionsServiceMock.removeAnimeFromFavorites).toHaveBeenCalledWith(
         'user-1', '1',
      );
   });
   ...
   it('GET /users/:id/favorites should return favorites list', async () => {
      userAnimeActionsServiceMock.getUserFavorites.mockResolvedValue([
         { id: 'fav-1', userId: 'user-1', externalAnimeId: '1', status: 'WATCHING' },
      ]);
   ...
   it('POST /users/:id/reviews should create a review', async () => {
      ...
      expect(userAnimeActionsServiceMock.rateAnime).toHaveBeenCalledWith(
         'user-1', '1', 9.5, expect.objectContaining({ comment: 'Excelente!' }),
      );
      expect(response.body).toEqual(
